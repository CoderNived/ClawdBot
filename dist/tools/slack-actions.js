...
exports.getConversationWith = getConversationWith;
exports.getThreadReplies = getThreadReplies;
exports.searchMessages = searchMessages;
exports.formatMessagesForContext = formatMessagesForContext;
exports.getMessagePermalink = getMessagePermalink;
...
const logger = (0, logger_js_1.createModuleLogger)('slack-actions');
// Initialize Slack client with logging
const webClient = new web_api_1.WebClient(index_js_1.config.slack.botToken, {
    logLevel: index_js_1.config.app.logLevel === 'debug' ? 'DEBUG' : 'INFO',
});
// User client for reminders (requires xoxp- token)
const userClient = index_js_1.config.slack.userToken
    ? new web_api_1.WebClient(index_js_1.config.slack.userToken)
    : null;
// Log the token prefix for debugging (safe - doesn't expose full token)
logger.info(`Slack client initialized with token: ${index_js_1.config.slack.botToken.substring(0, 10)}...`);
if (userClient) {
    logger.info(`User client initialized for reminders: ${index_js_1.config.slack.userToken.substring(0, 10)}...`);
}
else {
...
async function findUser(query) {
    try {
        ...
        const found = users.find((u) => u.name?.toLowerCase() === queryLower ||
 ...
 */
async function sendChannelMessage(channelId, message, threadTs) {
    try {
 ...
 */
async function getDMHistory(userId, limit = 50) {
    try {
        // Open/get DM channel
        const dmResult = await webClient.conversations.open({ users: userId });
        if (!dmResult.channel?.id) {
 ...
 */
async function getConversationWith(userName, limit = 50) {
 ...
 * Get thread replies
 */
async function getThreadReplies(channelId, threadTs, limit = 100) {
    try {
...
async function searchMessages(query, channelId, limit = 20) {
    try {
        ...
        for (const channel of memberChannels.slice(0, 10)) {
            ...
            const matches = history.filter((msg) => msg.text.toLowerCase().includes(queryLower));
 ...
 */
async function getMessagePermalink(channelId, messageTs) {
    try {
 ...
 */
async function scheduleMessage(target, message, sendAt) {
    try {
 ...
 */
async function deleteScheduledMessage(channelId, scheduledMessageId) {
    try {
 ...
 */
async function setReminder(userId, text, time) {
