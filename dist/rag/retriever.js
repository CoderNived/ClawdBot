...
const logger = (0, logger_js_1.createModuleLogger)('retriever');
/**
...
async function retrieve(query, options = {}) {
    ...
    const { limit = 10, minScore = 0.3, // Lowered default for better recall
    channelName, channelId, userId, includeContext = false, contextWindow = 2, } = options;
    logger.info(`Retrieving for query: "${query.substring(0, 50)}..."${channelName ? ` in #${channelName}` : ''}`);
    try {
        // 1. Preprocess and embed the query
        const processedQuery = (0, embeddings_js_1.preprocessText)(query) || query;
        logger.debug(`Processed query: "${processedQuery.substring(0, 50)}..."`);
        const queryEmbedding = await (0, embeddings_js_1.createEmbedding)(processedQuery);
        // 2. Search the vector store
        const searchResults = await (0, vectorstore_js_1.search)(queryEmbedding, {
